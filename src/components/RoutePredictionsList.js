...
const RoutePredictionsList = ({
    ...
    fetch(url)
      ...
      .then(d => {
        ...
        let tripAdded = d.data.entry.arrivalsAndDepartures.map(dp => {
          let filteredTrip = trips.filter(
            tr => dp.tripId.indexOf(tr.tripId) > -1
          )[0];
          return {
            trip: filteredTrip,
            ...dp
  ...
  let matchedPredictions = predictions.filter(
    pr => pr.routeId.indexOf(route.rt_id.toString()) > -1
  );
  return (
        ...
        {matchedPredictions.length > 0 ? (
              ...
              <ListItem
                ...
                onClick={() =>
                  currentTrip === p.tripId
                    ? handleChange(null)
                    : handleChange(p.tripId)
                }
              >
                <ListItemIcon>
                  {p.predicted ? <SpeakerPhone /> : <Schedule />}
                </ListItemIcon>
                <ListItemText
                  secondary={p.directionId}
                  primary={`${
                    p.predicted
                      ? moment(p.predictedArrivalTime).format("h:mma")
                      : moment(p.scheduledArrivalTime).format("h:mma")
                  }`}
                />
              </ListItem>
              <Collapse
                in={currentTrip === p.tripId}
                timeout="auto"
                unmountOnExit
              >
          ...
          <Card>
            <CardContent style={{ padding: "1em" }}>
              There are currently no real-time predictions available, see the
              schedule below.
            </CardContent>
          </Card>
        )}
